...
function getCsvAppInstance() {
    if (window.CsvApp && typeof window.CsvApp.handleFileUpload === 'function') {
        return window.CsvApp;
    }
    if (typeof CsvApp !== 'undefined' && CsvApp && typeof CsvApp.handleFileUpload === 'function') {
        return CsvApp;
    }
    return null;
}
...
const ToolRouter = {
    async routeFile(file) {
        ...
        if (['xlsx', 'xls'].includes(ext)) {
            const csvApp = getCsvAppInstance();
            if (!csvApp) throw new Error('Modulo CSV indisponivel no momento.');
...
window.ToolRouter = ToolRouter;
window.handleSmartCsvUpload = handleSmartCsvUpload;
