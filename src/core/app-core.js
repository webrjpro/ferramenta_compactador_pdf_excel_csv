...
// ══════════════════════════════════════════════════════════════
const Config = {
    MAX_ZIP_SIZE: 15 * 1024 * 1024,          // Limite estrito por ZIP: 15 MB
    ZIP_SAFETY_MARGIN: 96 * 1024,            // Margem para overhead estrutural do formato ZIP
    PDF_COMPRESS_THRESHOLD: 8 * 1024 * 1024, // Só tenta otimizar PDFs realmente grandes
...
const PDF = {
     ...
     */
    async compress(pdfBytes, onProgress, options = {}) {
        const renderScale = Number.isFinite(options.scale)
            ? Math.max(0.8, Math.min(2.0, options.scale))
            : 1.2;
        const jpegQuality = Number.isFinite(options.jpegQuality)
            ? Math.max(0.45, Math.min(0.95, options.jpegQuality))
            : 0.82;

        const { jsPDF } = window.jspdf;
        const loadingOptions = { data: pdfBytes };
        if (location.protocol === 'file:') {
            // Em file:// alguns browsers bloqueiam worker via blob:null.
            loadingOptions.disableWorker = true;
        }
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
            ...
            doc.addImage(canvas.toDataURL('image/jpeg', jpegQuality), 'JPEG', 0, 0, viewport.width, viewport.height);
    ...
    /**
     * Tenta manter o PDF abaixo do alvo com a melhor qualidade possível.
     * Aplica perfis progressivos: começa em alta qualidade e reduz somente quando necessário.
     * @param {File|Blob} sourceFile
     * @param {number} targetBytes
     * @param {Function} onProgress
     * @returns {Promise<{blob: Blob, targetMet: boolean}>}
     */
    async compressToTarget(sourceFile, targetBytes, onProgress) {
        const profiles = [
            { label: 'qualidade alta',        scale: 1.35, jpegQuality: 0.90 },
            { label: 'qualidade boa',         scale: 1.25, jpegQuality: 0.86 },
            { label: 'qualidade equilibrada', scale: 1.18, jpegQuality: 0.82 },
            { label: 'qualidade compacta',    scale: 1.10, jpegQuality: 0.76 },
            { label: 'compactacao forte',     scale: 1.00, jpegQuality: 0.70 },
            { label: 'compactacao extrema',   scale: 0.92, jpegQuality: 0.64 },
        ];
        ...
        if (originalBlob.size <= targetBytes) {
            return { blob: originalBlob, targetMet: true };
        }

        for (const profile of profiles) {
            onProgress(`Ajustando ${profile.label}...`);
            const compressedBytes = await this.compress(originalBytes, onProgress, profile);
            const candidateBlob = new Blob([compressedBytes], { type: 'application/pdf' });
            ...
            if (candidateBlob.size <= targetBytes) {
                return { blob: candidateBlob, targetMet: true };
            }
        }

        return { blob: bestBlob, targetMet: bestBlob.size <= targetBytes };
    },
};
...
const ZIP = {
    /**
     * Estima o custo total de um ficheiro no ZIP (dados + cabeçalhos ZIP).
     * O fator 2x no nome cobre local header + central directory.
     */
    estimateZipEntrySize(zipPath, fileSize) {
        const nameLen = new TextEncoder().encode(String(zipPath || '')).length;
        return fileSize + 76 + (nameLen * 2);
    },

    /**
     * Constrói os ZIPs de um aluno organizados por categoria.
     * Divide em partes para manter cada ZIP até 15 MB.
    ...
    buildCategoryOrdered(studentName, studentFiles) {
        ...
        const result = [];
        const EMPTY_ZIP_SIZE = 22; // End of central directory
        const maxEstimatedZipSize = Math.max(1, Config.MAX_ZIP_SIZE - Config.ZIP_SAFETY_MARGIN);
        let currentZip = new JSZip();
        let currentEstimatedSize = EMPTY_ZIP_SIZE;
        let partNumber = 1;
        ...
        for (const [catName, catFiles] of filesByCategory) {
            ...
            for (const item of catFiles) {
                ...
                const zipPath = `${catFolderName}/${item.originalName}`;
                const estimatedEntrySize = this.estimateZipEntrySize(zipPath, fSize);

                if (
                    currentEstimatedSize > EMPTY_ZIP_SIZE &&
                    (currentEstimatedSize + estimatedEntrySize) > maxEstimatedZipSize
                ) {
                    result.push({
                ...
                currentEstimatedSize += estimatedEntrySize;
            }
        }

        if (currentEstimatedSize > EMPTY_ZIP_SIZE || result.length === 0) {
            result.push({
                zipName: partNumber === 1 ? `${studentName}.zip` : `${studentName}_parte${partNumber}.zip`,
                zip: currentZip,
            });
    ...
    async generate() {
        ...
        try {
            ...
            for (let i = 0; i < names.length; i++) {
                ...
                const maxPdfTargetBytes = Math.max(1, Config.MAX_ZIP_SIZE - Config.ZIP_SAFETY_MARGIN);
                const oversizedPdfFiles = sFiles.filter((item) =>
                    item.originalName.toLowerCase().endsWith('.pdf') &&
                    item.file.size > Config.PDF_COMPRESS_THRESHOLD &&
                    item.file.size > maxPdfTargetBytes
                );
                const oversizedNonPdfFiles = sFiles.filter((item) =>
                    !item.originalName.toLowerCase().endsWith('.pdf') &&
                    item.file.size > maxPdfTargetBytes
                );

                // Etapa 1: otimizacao pontual de PDFs grandes (qualidade primeiro)
                if (oversizedPdfFiles.length > 0) {
                    DOM.statusText.textContent = `A otimizar PDFs grandes de: ${name}`;
                    DOM.fileProgressContainer.classList.remove('hidden-section');

                    for (const item of oversizedPdfFiles) {
                        const beforeMB = (item.file.size / 1024 / 1024).toFixed(1);

                        try {
                            const result = await PDF.compressToTarget(
                                item.file,
                                maxPdfTargetBytes,
                                (msg) => { DOM.fileProgressText.textContent = `${item.originalName}: ${msg}`; }
                            );
                            ...
                            if (item.file.size > maxPdfTargetBytes) {
                                UI.toast(
                                    `Aviso: "${item.originalName}" continua muito grande para o limite de 15MB.`,
                                    'info',
                                    9000
                                );
                            }
                ...
                if (oversizedNonPdfFiles.length > 0) {
                    UI.toast(
                        `${oversizedNonPdfFiles.length} arquivo(s) nao-PDF de "${name}" excedem 15MB e podem gerar ZIP acima do limite.`,
                        'info',
                        9000
                    );
                }
    ...
    configurePdfRuntime() {
        ...
        try {
            ...
            if (fileMode) {
                ...
                // Compatibilidade adicional com ambientes locais estritos.
                window.pdfjsLib.disableWorker = true;
                Logger.info('[PDF] Modo local detectado (file://): worker configurado com fallback seguro.');
            }
        } catch (err) {
            Logger.warn('[PDF] Falha ao configurar worker do PDF.js', err);
        }
    },
...
function initOptionalModule(moduleName) {
    const mod = window[moduleName];
    if (mod && typeof mod.init === 'function') {
        mod.init();
        return true;
    }
    Logger.warn(`[Bootstrap] Modulo "${moduleName}" nao encontrado ou sem init().`);
    return false;
}

async function invokeSmartCsvUpload(file) {
    if (!file) return;

    if (typeof window.handleSmartCsvUpload === 'function') {
        await window.handleSmartCsvUpload(file);
        return;
    }

    if (window.CsvApp && typeof window.CsvApp.handleFileUpload === 'function') {
        window.CsvApp.handleFileUpload(file);
        return;
    }

    throw new Error('Roteador de arquivos CSV indisponivel.');
}
    ...
    async _handleDrop(e) {
        ...
        if (typeof first.getAsFileSystemHandle === 'function') {
            try {
                ...
                if (handle.kind === 'file') {
                    ...
                    if (file.name.match(/\.(xlsx|xls|csv)$/i)) {
                        await invokeSmartCsvUpload(file);
                        return;
                    } else {
            ...
            if (dtFiles && dtFiles.length > 0 && dtFiles[0].name.match(/\.(xlsx|xls|csv)$/i)) {
                await invokeSmartCsvUpload(dtFiles[0]);
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        App.init();
        initOptionalModule('CsvApp');
        initOptionalModule('ReportApp');
        initOptionalModule('ZipadorWebApp');
    } catch (err) {
    ...
    if (csvInput) {
        csvInput.addEventListener('change', async (e) => {
            if (e.target.files && e.target.files.length > 0) {
                try {
                    await invokeSmartCsvUpload(e.target.files[0]);
                } catch (err) {
                    Logger.error('Falha ao rotear upload de CSV.', err);
